...
beforeAll(async () => {
  ...
  const adminMod = await loadRscModule<typeof import("../../src/server/admin/events")>(
    "/src/server/admin/events.ts",
  );
  adminActionId = extractActionId(adminMod.listEvents);
  const publicMod = await loadRscModule<typeof import("../../src/server/public/rsvp")>(
...
test("admin action id triggers authorize callback (returns 401 when denied)", async () => {
  ...
  const res = await handler(
    new Request(`http://x/@rsc/${encodeURIComponent(adminActionId)}`, { method: "POST" }),
  );
